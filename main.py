        ...
        for question in question_list:
            ...
            if "what" in question_LC:
                query = "((words:"+words+")^20 OR (lemmas:"+ lemmas+")^10 OR (synonymns:"+synonymns+")^10 OR \
                    (hypernyms:"+hypernyms+") OR (hyponyms:"+hyponyms+") OR (meronyms:"+meronyms+")  OR (holonyms:"+holonyms+") OR \
                    (rootOfSentence:"+rootOfSentence+") OR (stems:"+stems+")^10 AND (entities:"+entities+")^20)"
            else:
            ...
            if(not os.path.isfile('./result.csv')):
                 headerList=['Question', 'Article_id', 'Answer']
            ...
            with open("result.csv", 'a', newline='',encoding='utf8') as csvFile:   
                if len(results) == 0:
                    data=[ question, "NA", "Answer not found"]
                else:    
                    data=[question, str(list(results)[0]["id"].split("_")[0]), list(results)[0]["sentence"][0]]
